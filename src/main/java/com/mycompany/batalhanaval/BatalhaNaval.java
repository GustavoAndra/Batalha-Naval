...
private void exibirMapaAtaques(int oponente) {
    ...
    for (int i = 0; i < TAMANHO_MAPA; i++) 
    {
        ...
        for (int j = 0; j < TAMANHO_MAPA; j++) 
        {
            char c = mapasAtaques[oponente][i][j];
            if (c == 'X') 
            {
                System.out.print(c + " "); // Se for um tiro que acertou água
            } 
            else if (c == 'B')
             {
                System.out.print(c + " "); // Se for um tiro que acertou um navio
            }
             else if (c == ATINGIDO)
              {
                System.out.print(c + " "); // Se for um tiro que acertou uma parte do navio
            } 
        }
        System.out.println();
    }
...
private void realizarAtaque(int jogador, int oponente, Scanner scanner) {
    ...
    while (!jogadaValida) {
        ...
        while (true) {
            ...
            System.out.print("Entrada inválida. Digite novamente a coluna (A-J): ");
        }
        int coluna = colunaStr.charAt(0) - 'A';
