    ...
    while (!jogadaValida) {
        ...
        switch (mapas[oponente][linha][coluna]) {
            ...
            default:
                ...
                {
                    int tamanhoBarco = obterTamanhoBarco(oponente, linha, coluna);
                    System.out.println("Parabéns! Você acertou uma parte de um barco do adversário!");
                 if (tamanhoBarco > 1) 
                 {
                        System.out.println("Você afundou um barco de " + tamanhoBarco + " posições!");
                 }
                } 
...
private int obterTamanhoBarco(int jogador, int linha, int coluna) {
    boolean[][] visitado = new boolean[TAMANHO_MAPA][TAMANHO_MAPA];
    return calcularTamanhoBarco(jogador, linha, coluna, visitado);
}

private int calcularTamanhoBarco(int jogador, int linha, int coluna, boolean[][] visitado) {
    if (linha < 0 || linha >= TAMANHO_MAPA || coluna < 0 || coluna >= TAMANHO_MAPA || visitado[linha][coluna]) {
        return 0;
    }

    visitado[linha][coluna] = true;

    if (mapas[jogador][linha][coluna] != ATINGIDO && mapas[jogador][linha][coluna] != ATINGIDO) {
        return 0;
    }

    int tamanhoBarco = 1;

    tamanhoBarco += calcularTamanhoBarco(jogador, linha - 1, coluna, visitado);
    tamanhoBarco += calcularTamanhoBarco(jogador, linha + 1, coluna, visitado);
    tamanhoBarco += calcularTamanhoBarco(jogador, linha, coluna - 1, visitado);
    tamanhoBarco += calcularTamanhoBarco(jogador, linha, coluna + 1, visitado);
    ...
    while (!jogadaValida) {
        ...
        switch (mapas[oponente][linha][coluna]) {
            ...
            default:
                ...
                int tamanhoBarco = obterTamanhoBarco(oponente, linha, coluna);
                if (tamanhoBarco > 1) {
                    System.out.println("A máquina afundou um barco de " + tamanhoBarco + " posições!");
                } else {
                    System.out.println("A máquina afundou um dos seus barcos!");
                }
                jogadaValida = true; // A máquina realizou um ataque válido, então encerra o loop.
                break;
        }

        if (verificarFimDeJogo(oponente)) {
            System.out.println("A máquina afundou todos os seus barcos! Você perdeu o jogo!");
            jogadaValida = true; // Encerra o loop se o jogo terminou.
        }
    }
}
